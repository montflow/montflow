...
Vitest.describe("[runtime] Table.pick", () => {
  ...
  Vitest.it("should work in curried form", () => {
    ...
    const pickXY = Table.pick(["x", "y"] as const);
    const result = pickXY(input);

    Vitest.expect(result).toEqual({ x: 10, y: 20 });
    Vitest.expect(Table.keys(result)).toEqual(["x", "y"]);
  });

  Vitest.it("should handle empty keys array", () => {
    const input = { a: 1, b: 2 };
    const result = Table.pick(input, []);
  ...
  Vitest.describe("curried version", () => {
    ...
    Vitest.it("should work with curried form - all keys", () => {
      ...
      const pickAll = Table.pick(["foo", "baz"] as const);
      const result = pickAll(input);

      Vitest.expect(result).toEqual({ foo: "bar", baz: 42 });
      Vitest.expect(Table.keys(result)).toEqual(["foo", "baz"]);
    });
    ...
    Vitest.it("should work with curried form - complex objects", () => {
      ...
      const pickPersonalAndMeta = Table.pick(["personal", "meta"] as const);
    ...
    Vitest.it("should preserve original object in curried form", () => {
      const input = { a: 1, b: 2, c: 3, d: 4 };
      const pickAC = Table.pick(["a", "c"] as const);
...
Vitest.describe("[types] Table.pick", () => {
  Vitest.it("should be defined", () => {
    type Test = typeof Table.pick;
