...
import * as NumberExt from '../number-ext/index.js';
import * as RecordExt from '../record-ext/index.js';
...
export const isObject = (thing: unknown): thing is Object =>
  Predicate.isObject(thing) &&
  RecordExt.hasKeys(thing, ['min', 'max']) &&
...
export const symetric = (midpoint: number, margin: number): Range => {
  if (NumberExt.isNegative(margin)) throw new Error('Marging is negative');
