...
const EXT_DIR = '/home/daniel/dev/ai/main/pi/workspace'
const ROUTER_STATE = join(homedir(), '.local', 'state', 'workspace', 'router.json')
...
try {
  ...
  check('instance A has clean slug', lastFolders(b)[0]?.id === 'workspace', lastFolders(b)[0]?.id)
  check('hello for instance A', b.msgs.some((m) => m.type === 'hello' && m.folder === 'workspace'))

  // --- 4. command dispatch over WS (no agent turn) ---
  b.ws.send(JSON.stringify({ folder: 'workspace', command: { type: 'command', text: '/montflow --port=24342' } }))
  await sleep(1500)
  check('command dispatched (no agent turn)', !b.msgs.some((m) => m.type === 'event' && m.event?.type === 'agent_start'))

  // --- 5. prompt round-trip over WS ---
  b.ws.send(JSON.stringify({ folder: 'workspace', command: { type: 'prompt', text: 'hello from ws' } }))
  await sleep(2500)
  ...
  const inst2 = folders3.find((f) => f.id.startsWith('workspace--'))
  check('instance #2 has suffixed id', !!inst2, inst2?.id)
  check('instance #2 named #2', inst2?.name === 'workspace #2', inst2?.name)
  check('instance A still present (we did NOT stop it)', folders3.some((f) => f.id === 'workspace'))
  ...
  check('instance A survived', folders4.some((f) => f.id === 'workspace'))
