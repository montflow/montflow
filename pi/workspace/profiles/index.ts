 ...
 *
 * Profiles live at `.agents/@montflow/profiles/<name>/PROFILE.md` (structure defined by
...
export default function profilesExtension(pi: ExtensionAPI): void {
  ...
  pi.registerCommand('profiles', {
    description:
      'Store agent profiles (.agents/@montflow/profiles/<name>/PROFILE.md): ' +
  ...
  const handleNew = async (
    ...
    if (!complete && ctx.hasUI) {
      ...
      if (profile !== null) {
        ctx.ui.notify(`Profile created: .agents/@montflow/profiles/${profile.name}/PROFILE.md`, 'info');
      }
      return;
    }
    if (!complete) {
    ...
    ctx.ui.notify(`Profile created: .agents/@montflow/profiles/${name}/PROFILE.md`, 'info');
  };
}
