...
const previewAndSaveProfile = async (ctx: ExtensionContext, profile: Model.Profile): Promise<Model.Profile | null> => {
  ...
  const preview = await viewInEditor(ctx, `Preview: .agents/@montflow/profiles/${profile.name}/PROFILE.md`, markdown);
  if (preview === null) {
  ...
  ctx.ui.notify(`Profile created: .agents/@montflow/profiles/${finalProfile.name}/PROFILE.md`, 'info');
  return finalProfile;
};
...
const buildAgenticPrompt = async (prompt: string, ctx: ExtensionContext): Promise<string> => {
  ...
  return [
    ...
    prompt,
    '',
    'An agent profile is stored at `.agents/@montflow/profiles/<name>/PROFILE.md` and is data only:',
...
export const runEditWizard = async (ctx: ExtensionContext, name?: string): Promise<boolean> => {
  ...
  const edited = await viewInEditor(ctx, `Modify: .agents/@montflow/profiles/${profileName}/PROFILE.md`, markdown);
  ...
  ctx.ui.notify(`Saved: .agents/@montflow/profiles/${profileName}/PROFILE.md`, 'info');
  return true;
};
