...
test('statusLine: formats loop/cycle/phase/status', () => {
  expect(statusLine(0, 3, 1, 5, 'reviewer', 'running')).toBe(
    '● workspace [loop 1/3 · cycle 1/5] reviewer: running',
  );
  expect(statusLine(2, 3, 5, 5, 'fixer', 'running')).toBe(
    '● workspace [loop 3/3 · cycle 5/5] fixer: running',
  );
});
...
const withReviewDir = async (
  ...
  const reviewDir = path.join(dir.tmp, '.agents/@montflow/reviews/adversarial');
...
`;
    ...
    const statePath = path.join(dir.tmp, '.agents/@montflow/reviews/adversarial/001/loop-state.json');
...
test('runGraph: reviewers run concurrently and each writes its scratch file', () =>
  withReviewDir(async (dir, reviewFile) => {
    ...
    const scratchDir = path.join(dir.tmp, '.agents/@montflow/reviews/adversarial/001/passes/1/1/scratch');
...
`;
    ...
    expect(fs.existsSync(path.join(dir.tmp, '.agents/@montflow/reviews/adversarial/001/passes/1/1/fixes/F1.error.json'))).toBe(true);
...
`;
    ...
    fs.mkdirSync(path.join(dir.tmp, '.agents/@montflow/reviews/adversarial/001'), { recursive: true });
    fs.writeFileSync(
      path.join(dir.tmp, '.agents/@montflow/reviews/adversarial/001/loop-state.json'),
      JSON.stringify(state, null, 2),
    );

    // The interrupted fixer DID write its updated block (Status: In Review).
    const scratchDir = path.join(dir.tmp, '.agents/@montflow/reviews/adversarial/001/passes/1/1/fixes');
...
`;
    ...
    const fixesDir = path.join(dir.tmp, '.agents/@montflow/reviews/adversarial/001/passes/1/1/fixes');
...
`;
    ...
    for (const id of ['F1', 'F3']) {
      expect(
        fs.existsSync(
          path.join(dir.tmp, '.agents/@montflow/reviews/adversarial/001/passes/1/1/fixes', `${id}.error.json`),
        ),
      ).toBe(true);
    }
  }));
...
test('runGraph: opts.reviewFile escaping .agents/@montflow/reviews/ fails the loop', async () => {
  const dir = withProjectRoot({});
  try {
    ...
    expect(result.terminal).toBe('failed');
    expect(ui.notify).toHaveBeenCalledWith(
      expect.stringContaining('escapes .agents/@montflow/reviews'),
      'error',
    );
...
test('runGraph: opts.reviewFile escaping via a symlink under .agents/@montflow/reviews/ fails the loop', async () => {
  const dir = withProjectRoot({});
  try {
    // `.agents/@montflow/reviews/link` → outside dir; the review file does not exist
    ...
    // to catch the escape.
    const reviews = path.join(dir.tmp, '.agents/@montflow/reviews');
