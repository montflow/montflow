...
test('resolveReviewFile: non-existent directory creates it and returns 001.md', () =>
  withTempDir(async ({ tmp }) => {
    ...
    expect(result).toMatch(/\.agents\/@montflow\/reviews\/myreview\/001\.md$/);
    expect(fs.existsSync(path.join(tmp, '.agents/@montflow/reviews/myreview'))).toBe(true);
  }));

test('resolveReviewFile: empty directory (no .md files) returns 001.md', () =>
  withTempDir(async ({ tmp }) => {
    const dir = path.join(tmp, '.agents/@montflow/reviews/myreview');
...
test('resolveReviewFile: existing reviews reuse highest code (re-review)', () =>
  withTempDir(async ({ tmp }) => {
    ...
    const result = await runEffect(resolveReviewFile(tmp, 'myreview', false));
    expect(result).toMatch(/\.agents\/@montflow\/reviews\/myreview\/002\.md$/);
  }));
...
test('resolveReviewFile: fresh flag forces next number', () =>
  withTempDir(async ({ tmp }) => {
    ...
    expect(result).toMatch(/\.agents\/@montflow\/reviews\/myreview\/003\.md$/);
  }));
...
test('isWithinReviews: rejects a candidate escaping via a symlinked parent', () =>
  withTempDir(async ({ tmp }) => {
    const reviews = path.join(tmp, '.agents/@montflow/reviews');
...
test('isWithinReviews: a reviews root that is itself a symlink is compared by its real target', () =>
  withTempDir(async ({ tmp }) => {
    ...
    fs.mkdirSync(path.join(tmp, '.agents', '@montflow'), { recursive: true });
    fs.symlinkSync(realRoot, path.join(tmp, '.agents', '@montflow', 'reviews'), 'dir');
    ...
    await withNodePath((p) => {
      ...
      expect(isWithinReviews(p, tmp, path.join(tmp, '.agents', '@montflow', 'reviews', 'myreview', '001.md'))).toBe(true);
