...
test('setLoopWidget: pushes a component factory that renders the state', () => {
  ...
  expect(setWidget).toHaveBeenCalledWith('workspace', expect.any(Function));
  ...
  expect(setWidget).toHaveBeenCalledWith('workspace', undefined);
});
...
test('renderLoopWidget: focused view shows the agent stream tail + hint', () => {
  ...
  expect(joined).toContain('/workspace-focus off');
