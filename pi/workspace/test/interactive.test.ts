...
const seedPreset = (d: string): void => {
  fs.mkdirSync(path.join(d, '.agents/@montflow/review-presets'), { recursive: true });
  fs.writeFileSync(path.join(d, '.agents/@montflow/review-presets/x.json'), '{}');
};
...
/** Writes a profile to disk under `.agents/@montflow/profiles/<name>/`. */
const seedProfile = (d: string, profile: Profile): void => {
  const root = path.join(d, '.agents', '@montflow', 'profiles', profile.name);
  fs.mkdirSync(root, { recursive: true });
  fs.writeFileSync(path.join(root, 'PROFILE.md'), serializeProfile(profile), 'utf8');
};
...
test('create preset: added profile reviewer lands in the roster and menu reflects it', async () => {
  ...
  const stored = JSON.parse(
    fs.readFileSync(path.join(dir, '.agents/@montflow/review-presets/roster-audit.json'), 'utf8'),
...
test('create preset: fixer/supervisor thinking levels persist into the preset file', async () => {
  ...
  const stored = JSON.parse(
    fs.readFileSync(path.join(dir, '.agents/@montflow/review-presets/thinking-demo.json'), 'utf8'),
  ) as {
...
test('create preset: per-reviewer thinking level persists into the stored reference', async () => {
  ...
  expect(setup).toBeNull();
  const stored = JSON.parse(
    fs.readFileSync(path.join(dir, '.agents/@montflow/review-presets/thinking-reviewer.json'), 'utf8'),
...
test('preset flow: create a preset, then run a review with it via New review', async () => {
  ...
  const presetFile = path.join(dir, '.agents/@montflow/review-presets/my-preset.json');
...
test('preset flow: profile reviewers are stored as references and resolved on use', async () => {
  ...
  const stored = JSON.parse(
    fs.readFileSync(path.join(dir, '.agents/@montflow/review-presets/ref-preset.json'), 'utf8'),
  ) as { config: { reviewers: unknown[] } };
  expect(stored.config.reviewers).toEqual([
    { type: 'builtin', id: 'generic' },
    { type: 'profile', name: 'security-auditor', model: 'deepseek/deepseek-v4-pro' },
...
test('preset flow: creating an existing preset name refuses and aborts', async () => {
  ...
  const presetFile = path.join(dir, '.agents/@montflow/review-presets/existing.json');
...
test('back: new review — scope ← Back returns to the action menu', async () => {
  ...
  const presetFile = path.join(dir, '.agents/@montflow/review-presets/scope-back.json');
  fs.mkdirSync(path.dirname(presetFile), { recursive: true });
  fs.writeFileSync(
    presetFile,
    JSON.stringify({
...
test('back: settings ← Back returns to the roster (create preset)', async () => {
  ...
  const presetFile = path.join(dir, '.agents/@montflow/review-presets/settings-back.json');
  expect(fs.existsSync(presetFile)).toBe(true);
});
...
test('action menu: Resume review appears only when reviews exist', async () => {
  ...
  expect(
    await recordMenu((d) => {
      fs.mkdirSync(path.join(d, '.agents/@montflow/reviews/adversarial'), { recursive: true });
      fs.writeFileSync(path.join(d, '.agents/@montflow/reviews/adversarial/001.md'), '# Review\n');
    }),
  ).toEqual(['Preset']);
...
test('resume flow: uses the review locked-in config (no preset pick)', async () => {
  ...
  const reviewDir = path.join(dir, '.agents/@montflow/reviews/adversarial');
...
test('resume flow: legacy review without config snapshot falls back to a preset', async () => {
  ...
  const presetFile = path.join(dir, '.agents/@montflow/review-presets/resume-preset.json');
  fs.mkdirSync(path.dirname(presetFile), { recursive: true });
  fs.writeFileSync(
    presetFile,
    JSON.stringify({
...
test('back: inside create preset, ← Back abandons without saving', async () => {
  ...
  const presetFile = path.join(dir, '.agents/@montflow/review-presets/x.json');
...
test('preset flow: after create, Exit leaves the wizard', async () => {
  ...
  const presetFile = path.join(dir, '.agents/@montflow/review-presets/exit-preset.json');
  expect(fs.existsSync(presetFile)).toBe(true);
});
...
test('preset flow: list → view → modify re-renders the graphic and saves back', async () => {
  ...
  const presetFile = path.join(dir, '.agents/@montflow/review-presets/view-me.json');
  fs.mkdirSync(path.dirname(presetFile), { recursive: true });
  fs.writeFileSync(
    presetFile,
    JSON.stringify({
...
test('preset flow: list → delete removes the preset from the view', async () => {
  ...
  const presetFile = path.join(dir, '.agents/@montflow/review-presets/delete-me.json');
  fs.mkdirSync(path.dirname(presetFile), { recursive: true });
  fs.writeFileSync(
    presetFile,
    JSON.stringify({
