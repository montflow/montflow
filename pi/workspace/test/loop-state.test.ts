...
test('loop state: pass paths are keyed by loop then cycle (1-based)', () => {
  const reviewFile = '/tmp/.agents/@montflow/reviews/adversarial/001.md';
  const dir = passDir(reviewFile, 2, 3);
  expect(dir).toBe('/tmp/.agents/@montflow/reviews/adversarial/001/passes/2/3');
  ...
  expect(loopStatePath(reviewFile)).toBe(
    '/tmp/.agents/@montflow/reviews/adversarial/001/loop-state.json',
  );
});
