...
description: Scopes an adversarial review pass — drafts the brief for the reviewer roster and aggregates their scratch reports into one canonical review. Always invoked by the workspace orchestrator.
...
Invoked by the workspace orchestrator on **every** review pass — the supervisor is always on and aggregation is always agent-driven (never programmatic merge).
