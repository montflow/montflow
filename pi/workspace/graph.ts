...
import { broadcastLoopState } from './ui-server';
import {
  clearLoopWidget,
  phaseLabel,
  setLoopWidget,
 ...
 * command (`/workspace-focus`). Carries the stream store, the
...
const stopResult = (ctx: GraphCtx): Effect.Effect<NodeResult, never, FileSystem | Path> =>
  Effect.gen(function* () {
    yield* notify(ctx.ui, '[workspace] Stopped by user request.', 'warning');
...
export const statusLine = (
  ...
  `● workspace [loop ${loop + 1}/${maxLoops} · cycle ${cycle}/${maxCycles}] ${phaseLabel(phase)}: ${status}`;
...
const withWidget = (
  ...
  // Push structured state to connected web UI browsers (no-op when UI is off).
  broadcastLoopState(widget);
...
const skillGate: GraphNode = (ctx, deps) =>
  Effect.gen(function* () {
    ...
    yield* notify(
      ctx.ui,
      '[workspace] Verified: all skills referenced by the resolved config are present.',
      'info',
    );
...
const resolveCtx: GraphNode = (ctx) =>
  Effect.gen(function* () {
    ...
    // and pass scratch files can never be written outside `.agents/@montflow/reviews/`:
    ...
    if (!isWithinReviews(path, cwd, candidate)) {
      return yield* Effect.fail(
        new ReviewPathError({
          message: `Review file escapes .agents/@montflow/reviews/: ${candidate}`,
        }),
      );
    }
    const resolvedReviewFile = path.resolve(candidate);
    // Expose the canonical review path on the active-loop handle so the
    // findings browser (/workspace-findings, ctrl+shift+i) can
    ...
    if (reReview) {
      ...
      if (isAllTerminal(existingSummary)) {
        ...
        if (loop + 1 < config.maxLoops) {
          ...
          yield* notify(
            ui,
            `[workspace] Loop ${loop + 1} reached consensus — resuming at loop ${loop + 2} with fresh reviewers.`,
            'info',
          );
          ...
          yield* notify(
            ui,
            '[workspace] Existing review is already all-terminal — nothing to do.',
            'info',
          );
          yield* setStatus(ui, '● workspace DONE');
          clearLoopWidget(ui);
          return {
            next: null,
            ctx: {
    ...
    yield* notify(
      ui,
      `[workspace] file=${resolvedReviewFile} reReview=${reReview} ` +
    ...
    if (resumeAtFixer) {
      yield* notify(
        ui,
        `[workspace] Resuming at the fixer phase (loop ${loop + 1}, cycle ${loopState.cycle + 1}) — reviewers already ran; recovering fixer checkpoints and re-dispatching failed findings.`,
        'info',
      );
    }
...
const advanceLoop = (
  ...
  Effect.gen(function* () {
    ...
    if (reviewFile !== undefined && ctx.loopState !== undefined) {
      ...
      yield* notify(
        ctx.ui,
        `[workspace] Loop ${loop + 1}/${config.maxLoops} — spawning a fresh set of independent reviewers.`,
        'info',
      );
  ...
  Effect.gen(function* () {
    ...
    if (!allSucceeded) {
      ...
      if (failures >= MAX_CONSECUTIVE_FAILURES) {
        ...
        yield* setStatus(ui, '● workspace FAILED');
        clearLoopWidget(ui);
        return {
          next: null,
          ctx: {
    ...
    if (deadlockResult.newlyDeadlocked.length > 0) {
      yield* notify(
        ui,
        `[workspace] Deadlock escalated: ${deadlockResult.newlyDeadlocked.join(', ')}`,
        'warning',
      );
    }
    ...
    if (transition === 'consensus') {
      ...
      if (loop + 1 < config.maxLoops) {
        ...
        yield* notify(
          ui,
          `[workspace] Loop ${loop + 1} consensus — all findings resolved or dismissed. Advancing to loop ${loop + 2} with fresh reviewers.`,
          'info',
        );
      ...
      yield* notify(
        ui,
        '[workspace] Completed: all loops reached consensus — all findings resolved or dismissed.',
        'info',
      );
      clearLoopWidget(ui);
    ...
    if (transition === 'escalated') {
      yield* notify(
        ui,
        `[workspace] ${Option.getOrElse(summary, () => undefined)?.escalated} escalated finding(s) need human input. Review file: ${reviewFile}`,
        'warning',
      );
      yield* setStatus(ui, '● workspace ESCALATED');
      clearLoopWidget(ui);
...
const fixer: GraphNode = (ctx, deps) =>
  Effect.gen(function* () {
    ...
    if (recovered.length > 0) {
      yield* notify(
        ui,
        `[workspace] Recovered ${recovered.length} fixer checkpoint(s) (${recovered.join(', ')}) from a previous interrupted pass — merged without re-dispatching.`,
        'info',
      );
...
const cycleMaxDecision: GraphNode = (ctx, deps) =>
  Effect.gen(function* () {
    ...
    if (deps.askUser === undefined) {
      yield* notify(
        ctx.ui,
        `[workspace] Loop ${loop + 1} reached the cycle cap (${config.maxCycles}) without consensus — ` +
          `open ${open} · in-review ${inReview} · escalated ${escalated}. Review file: ${ctx.reviewFile}`,
        'warning',
      );
      yield* setStatus(ctx.ui, '● workspace MAX LOOPS');
    ...
    if (choice === 'Increase cycle max') {
      ...
      yield* notify(
        ctx.ui,
        `[workspace] Cycle cap raised to ${maxCycles} — continuing loop ${loop + 1} with the same reviewers.`,
        'info',
      );
      return {
        next: 'review',
    ...
    if (choice === 'Resume to NEXT loop' || choice === 'Add a new loop') {
      ...
      if (choice === 'Add a new loop') {
        ...
        yield* notify(
          ctx.ui,
          `[workspace] Added a new loop — max loops is now ${newConfig.maxLoops}.`,
          'info',
        );
        effectiveCtx = {
          ...ctx,
    ...
    yield* notify(ctx.ui, '[workspace] Stopped by user decision.', 'info');
    yield* setStatus(ctx.ui, undefined);
    clearLoopWidget(ctx.ui);
    return { next: null, ctx: { ...ctx, terminal: 'stopped' as const } };
  });
...
/**
 * Runs the workspace state machine until a terminal node.
...
export const runGraph = (
  ...
  Effect.suspend(() => {
    ...
    // handle the focus command (/workspace-focus) reads/re-pushes.
          ...
          Effect.matchEffect({
            ...
            onFailure: (error) =>
              Effect.gen(function* () {
                yield* notify(
                  ctx.ui,
                  `[workspace] Unexpected error in '${nodeName}': ${error.message}`,
                  'error',
                );
                yield* setStatus(ctx.ui, '● workspace FAILED');
                clearLoopWidget(ctx.ui);
                return { next: null, ctx: { ...ctx, terminal: 'failed' as const } };
              }),
          }),
