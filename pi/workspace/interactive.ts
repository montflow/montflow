 ...
 *
 * `/workspace` walks the user through:
 ...
 *      the user add stored profiles (searched from the merged profiles store at
 *      `.agents/@montflow/profiles/`), change per-reviewer models (searchable
 *      list of the session's available models, preselected with the current
 *      model), and remove reviewers.
...
const pickAction = async (ctx: ExtensionContext): Promise<SetupAction | null> => {
  ...
  const pick = await ctx.ui.select(
    'workspace — what would you like to do?',
    options,
  );
...
const buildRoster = async (
  ...
  for (;;) {
    ...
    if (pick === '+ Add reviewer (profile)') {
      const added = await addProfileReviewer(ctx, roster);
      if (added !== null) {
 ...
 * only — then persists the configuration to `.agents/@montflow/review-presets/`.
 * @param {ExtensionAPI} pi The Pi extension API
 * @param {ExtensionContext} ctx The command context
...
/**
 * Lists the existing review files under `.agents/@montflow/reviews/adversarial/`
...
const listReviews = (cwd: string): readonly ReviewEntry[] => {
  const dir = path.join(cwd, '.agents/@montflow/reviews', 'adversarial');
...
const pickReview = async (ctx: ExtensionContext): Promise<ReviewEntry | null> => {
  ...
  if (reviews.length === 0) {
    ctx.ui.notify('No reviews in .agents/@montflow/reviews/adversarial/ to resume.', 'info');
    return null;
  }
...
const modifyReviewConfig = async (
  ...
  for (;;) {
    ...
    if (!saveReviewConfig(reviewFile, next)) {
      ctx.ui.notify(
        `[workspace] Could not write the updated config to ${loopStatePath(reviewFile)}.`,
        'error',
      );
      return null;
    }
    ctx.ui.notify(
      '[workspace] Review config updated (roster, models, fallback chains) — resuming with it.',
      'info',
    );
    return next;
  }
...
const pickPreset = async (ctx: ExtensionContext): Promise<string | null> => {
  ...
  if (names.length === 0) {
    ctx.ui.notify(
      'No presets stored in .agents/@montflow/review-presets/. Create one first.',
      'info',
    );
    return null;
  }
