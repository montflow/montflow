 ...
 * timeout error when the profiles extension is not loaded.
 */
export const getProfileViaBus = (
  ...
  return new Promise((resolve) => {
    const timer = setTimeout(() => {
      off();
      resolve({ id, ok: false, error: `profiles:get timed out after ${timeoutMs}ms (profiles extension not loaded?)` });
    }, timeoutMs);
...
export const listProfilesViaBus = (
  ...
  return new Promise((resolve) => {
    const timer = setTimeout(() => {
      off();
      resolve({ id, ok: false, error: `profiles:list timed out after ${timeoutMs}ms (profiles extension not loaded?)` });
    }, timeoutMs);
