...
const PROFILES_DIR = ['.agents', '@montflow', 'profiles'];
const PROFILE_FILE = 'PROFILE.md';
const TEMPLATE_FILE = 'TEMPLATE.md';

/** Path service helpers: `path.join(cwd, '.agents', '@montflow', 'profiles')`. */
const profilesRoot = (path: Path, cwd: string): string => path.join(cwd, ...PROFILES_DIR);

/**
 * Ensures `.agents/@montflow/profiles/` exists, and seeds `TEMPLATE.md` into it when
...
/** Lists profile names: directories under `.agents/@montflow/profiles/` that contain PROFILE.md. */
