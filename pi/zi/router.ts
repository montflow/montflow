...
/** Tell every browser that a prompt changed in a workspace (created/updated/deleted). */
const broadcastPromptChanged = (
  cwd: string,
  workspaceId: string,
  promptName: string,
  kind: 'created' | 'updated' | 'deleted',
...
const writePresetFile = async (
  ...
  // Every preset file declares its format version — refuse to write one
  // without it (or with a version we don't know) rather than silently
  // stamping/rewriting, so the stored JSONs always carry the version they
  // were authored with.
  if (bodyObj.version !== 1) {
    return {
      ok: false,
      error: `Unsupported preset version '${String(bodyObj.version)}' — only version 1 is supported`,
    };
  }
...
// ---------------------------------------------------------------------------
// Prompts — `.agents/@montflow/prompts/<name>.json`
// ---------------------------------------------------------------------------

const PROMPT_DIR = ['.agents', '@montflow', 'prompts'] as const;
const PROMPT_EXT = '.json';

const isValidPromptName = (name: string): boolean =>
  /^[a-zA-Z0-9][a-zA-Z0-9._-]*$/.test(name);

const promptRoot = (cwd: string): string => join(cwd, ...PROMPT_DIR);
const promptFilePath = (cwd: string, name: string): string =>
  join(promptRoot(cwd), `${name}${PROMPT_EXT}`);
...
const listPrompts = async (cwd: string): Promise<PromptSummary[]> => {
  let entries: string[];
  try {
    entries = await readdir(promptRoot(cwd));
  } catch {
    return []; // directory missing → no prompts yet
  }
  const results: PromptSummary[] = [];
  for (const entry of entries.filter((e) => e.endsWith(PROMPT_EXT))) {
    const name = entry.slice(0, -PROMPT_EXT.length);
    try {
      const raw = await readFile(promptFilePath(cwd, name), 'utf8');
      const prompt = Schema.decodeSync(PromptFromJson)(raw);
      results.push({
        name,
  ...
  results.toSorted((a, b) => a.name.localeCompare(b.name));
  return results;
};
...
const writePromptFile = async (
  ...
  if (!isValidPromptName(name)) {
    return { ok: false, error: `Invalid prompt name: '${name}'` };
  }
  const bodyObj =
    typeof body === 'object' && body !== null ? (body as Record<string, unknown>) : {};
  // Every prompt file declares its format version — refuse to write one
  // without it (or with a version we don't know) rather than silently
  // stamping/rewriting, so the stored JSONs always carry the version they
  // were authored with.
  if (bodyObj.version !== 1) {
    return {
      ok: false,
      error: `Unsupported prompt version '${String(bodyObj.version)}' — only version 1 is supported`,
    };
  }
  const prompt = {
    version: 1 as const,
    ...
    skills: bodyObj.skills,
  };
  try {
    Schema.decodeUnknownSync(PromptSchema)(prompt);
  } catch (error) {
    return { ok: false, error: error instanceof Error ? error.message : String(error) };
  }
  try {
    await mkdir(promptRoot(cwd), { recursive: true });
    await writeFile(promptFilePath(cwd, name), `${JSON.stringify(prompt, null, 2)}\n`, 'utf8');
    return { ok: true };
  } catch (error) {
    return { ok: false, error: error instanceof Error ? error.message : String(error) };
  }
...
const readPromptFile = async (cwd: string, name: string): Promise<string | null> => {
  try {
    return await readFile(promptFilePath(cwd, name), 'utf8');
  } catch {
    return null;
  }
};

const deletePromptFile = async (cwd: string, name: string): Promise<void> => {
...
async function main(): Promise<void> {
  ...
  const httpServer: Server = createServer(async (req, res) => {
    ...
    try {
      ...
      // Prompts — parameterized prompt templates + variables, keyed by
      // workspace id (HTTP API backups, see router.ts HELP text above).
      // GET  /api/workspaces/<id>/prompts                  → list
      // GET  /api/workspaces/<id>/prompts/<name>.json      → read one
      // POST /api/workspaces/<id>/prompts/<name>.json      → create/overwrite
      // DELETE /api/workspaces/<id>/prompts/<name>.json    → delete
      const promptList = pathname.match(/^\/api\/workspaces\/([^/]+)\/prompts$/);
      const promptFile = pathname.match(/^\/api\/workspaces\/([^/]+)\/prompts\/([^/]+)$/);

      if (promptList) {
        const wsId = decodeURIComponent(promptList[1] ?? '');
        const cwd = workspacePath(wsId);
        if (cwd === undefined) {
          sendJson(404, { error: `Unknown workspace '${wsId}'` });
          return;
        ...
        if (req.method === 'GET') {
          sendJson(200, { prompts: await listPrompts(cwd) });
          return;
        }
        sendJson(405, { error: 'Method not allowed' });
        return;
      ...
      if (promptFile) {
        const wsId = decodeURIComponent(promptFile[1] ?? '');
        const name = decodeURIComponent(promptFile[2] ?? '').replace(/\.json$/, '');
        const cwd = workspacePath(wsId);
        if (cwd === undefined) {
          sendJson(404, { error: `Unknown workspace '${wsId}'` });
          return;
        ...
        if (req.method === 'GET') {
          ...
          if (raw === null) {
            sendJson(404, { error: 'Prompt not found' });
            return;
          }
          res.writeHead(200, { 'content-type': 'application/json' });
          res.end(raw);
        ...
        if (req.method === 'POST') {
          ...
          // Distinguish create vs update for the cross-tab broadcast.
          const existed = await readPromptFile(cwd, name).then((raw) => raw !== null).catch(() => false);
          const result = await writePromptFile(cwd, name, body);
          if (result.ok) {
            broadcastPromptChanged(cwd, wsId, name, existed ? 'updated' : 'created');
            sendJson(200, { ok: true, name });
            return;
          }
          sendJson(400, { error: result.error });
        ...
        if (req.method === 'DELETE') {
          const existed = await readPromptFile(cwd, name).then((raw) => raw !== null).catch(() => false);
          await deletePromptFile(cwd, name);
          if (existed) broadcastPromptChanged(cwd, wsId, name, 'deleted');
          sendJson(200, { ok: true });
          return;
        }
        sendJson(405, { error: 'Method not allowed' });
        ...
        if (
          ...
          socket.send(
            JSON.stringify({
              type: 'notify',
              folder: msg.folder,
              message: 'No live pi session for this folder — run /montflow ui there.',
              level: 'warning',
            } satisfies RouterToBrowser),
          );
        }
