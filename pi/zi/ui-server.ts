...
export const startUiServer = async (
  ...
  type AgenticRunKind = 'skill' | 'profile' | 'preset' | 'text' | 'prompt';
  ...
  interface StartAgenticRunOptions {
    ...
    profileName?: string;
    /** Saved prompt name for prompt runs (used for run-title prefixing). */
    promptName?: string;
    ...
    void (async () => {
      ...
      const authoringSkill =
        kind === 'skill' && opts.useAuthoringSkill === true
          ...
          : undefined;
      // Prompt runs may attach workspace skills — load their SKILL.md bodies
      // so they ride along as instructions in the wrapped prompt.
      const promptSkills =
        kind === 'prompt' && (opts.skills?.length ?? 0) > 0
          ? await loadPromptSkills(cwd, opts.skills ?? [])
          : undefined;
      const prompt =
        kind === 'skill'
          ...
          : kind === 'profile'
            ...
            : kind === 'preset'
              ...
              : kind === 'prompt'
                ? wrapPromptPrompt(opts.text, opts.promptName, promptSkills)
        ...
        kind === 'skill'
          ...
          : kind === 'profile'
            ...
            : kind === 'preset'
              ...
              : kind === 'prompt'
                ? createPromptAgent(ctx, model, { sessionDir: runDir(cwd, runId) })
  ...
  const handleCommand = (command: BrowserCommand): void => {
    ...
    switch (command.type) {
      ...
      case 'promptAgentic': {
        // Run a saved workspace prompt — the browser rendered the template
        // with its variables filled in; the agent executes the rendered text
        // as its task in an isolated, persistent run (full toolset, so a
        // prompt may read, write, or fix). `promptName` names the source
        // prompt for the run-title prefix.
        startAgenticRun('prompt', {
          runId: command.runId,
          text,
