...
/** URL for a prompt details page: /w/<workspace-id>/prompts/<name>/ */
export const promptUrl = (workspaceId: string, promptName: string): string =>
  `/w/${encodeURIComponent(workspaceId)}/prompts/${encodeURIComponent(promptName)}/`
...
/** Extract the loop id from a /w/<id>/loops/<loop-id>/ pathname (or null). */
export const loopIdFromPath = (pathname: string): string | null => {
  const match = decodeURIComponent(pathname).match(/^\/w\/[^/]+\/loops\/([^/]+)\/?$/)
  return match?.[1] ?? null
}

/** Extract the prompt name from a /w/<id>/prompts/<name>/ pathname (or null). */
export const promptNameFromPath = (pathname: string): string | null => {
  const match = decodeURIComponent(pathname).match(/^\/w\/[^/]+\/prompts\/([^/]+)\/?$/)
  return match?.[1] ?? null
}
