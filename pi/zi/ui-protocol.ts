...
export const routerStateDir = (): string => {
  if (process.platform === 'win32') {
    return join(process.env.LOCALAPPDATA ?? homedir(), 'zi');
  }
  const xdg = process.env.XDG_STATE_HOME;
  return xdg && xdg !== ''
    ? join(xdg, 'zi')
    : join(homedir(), '.local', 'state', 'zi');
};
...
export type RouterToBrowser =
  ...
  // A prompt was created/updated/deleted in a workspace (HTTP API
  // mutations); browsers invalidate their prompt queries so every tab stays
  // in sync.
  | {
      readonly type: 'promptChanged';
      readonly folder: string | null;
      readonly workspaceId: string;
      readonly promptName: string;
      readonly kind: 'created' | 'updated' | 'deleted';
