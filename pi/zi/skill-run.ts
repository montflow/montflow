  ...
  "config": {
    "steps": [
      { "id": "s1", "kind": "reviewer-group", "label": "Reviewers", "concurrency": 3,
        ...
        "reviewers": [
          { "type": "builtin", "id": "generic", "model": "deepseek-v4-flash-free" },
...
- Reviewer refs: type "builtin" uses the catalog id (generic, security,
  ...
  confirm it exists before referencing it. Each ref MUST carry a model
  (the preset is invalid without it — reviewers never fall back to a
  profile/header default); fallbackModel (a single model id, only valid
  when model is set) and thinkingLevel (off|minimal|low|medium|high|xhigh|max)
  are optional.
...
/**
 * Create a fresh, isolated agent for a prompt run (full toolset — read AND
 * write). The rendered prompt is the task, so the agent can investigate or
 * apply changes depending on what the prompt asks. Shares the
 ...
 * workspace prompt (its template with variables filled in). It can read
 * AND write, so a prompt may ask for edits, fixes, or generated files — the
 * prompt text is the task.
 */
export const PROMPT_RUNNER_SYSTEM = `You are a coding agent executing a saved workspace prompt.

The user will give you a fully rendered prompt — a workspace template whose
variables have already been filled in. Execute it exactly as your task: use
your tools to read and (when asked) write or edit files in the workspace,
and report concisely what you did.

Rules:
- Do whatever the prompt asks — investigate, analyze, draft, fix, or write —
  using the workspace as your ground truth.
- Only touch what the prompt targets; stay in scope.
- If the prompt is genuinely ambiguous, ask ONE concise clarifying question
  and END it with a single "?" (the UI uses it to show the run is waiting
  for the user).
- When done, summarize in one short line what you accomplished.`;

/**
 * Wrap a fully rendered prompt for the run agent: the rendered text plus a
 * short instruction slotting it in as the task. When the source prompt
 * name is known it is included for context/titles. Attached skills'
 * SKILL.md bodies are appended as instructions the agent should follow.
 */
...
/** Extract the `name:` field from a skill's frontmatter block (or undefined). */
const skillNameOf = (markdown: string): string | undefined => {
  const match = /^---\n([\s\S]*?)\n---/.exec(markdown);
  const frontmatter = match?.[1];
  if (frontmatter === undefined) return undefined;
  for (const line of frontmatter.split('\n')) {
    if (/^name\s*:/i.test(line)) {
      return line.replace(/^name\s*:\s*/i, '').trim() || undefined;
    }
  }
  return undefined;
};
...
/**
 * Read the SKILL.md bodies for the requested skill names from
 * `.agents/skills/`. Names are matched against each file's frontmatter
 * `name` (the picker stores those; a directory whose name matches directly
 * is used too). Only the requested skills that were found are returned.
 */
export const loadPromptSkills = async (
  cwd: string,
  ...
  for (const entry of entries) {
    ...
    let body: string;
    try {
      body = await readFile(join(root, entry.name, 'SKILL.md'), 'utf8');
    } catch {
      continue;
    }
    const name = skillNameOf(body) ?? entry.name;
    if (wanted.has(name) && !found.has(name)) found.set(name, body);
  }
  return [...found.entries()].map(([name, body]) => ({ name, body }));
};
