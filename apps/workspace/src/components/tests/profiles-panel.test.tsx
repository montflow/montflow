...
const row = (id: string): Profiles.ProfileSummary => ({
  id,
  name: id,
  description: '',
  model: '',
...
const check = (actual: boolean, label: string): void => {
  if (!actual) throw new Error(`ProfilesPanel layout: ${label}`);
};
...
const states = () =>
  new Map([
    ...
    ['loadingProfiles', { loading: true, loadingVariant: 'profiles' as const }],
